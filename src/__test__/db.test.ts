import {
    ...
    joinClassFromClassId
} from "@/db/classes";
import {
    getInstructorsFromClassId,
    getStudentsFromClassId,
    getTAsFromClassId,
    getUserFromUserId,
    getUsers
} from "@/db/users";
...
describe("Database Setters", () => {
    it("createAndJoinClass", async () => {
        await expect(
            createAndJoinClass("1", "Machine Learning", "ML 1234", "Fall 2021")
        ...
        await expect(
            createAndJoinClass("2", "Data Structures", "DS 1234", "Fall 2021")
        ).resolves.toMatchObject({
    ...
    it("joinClassFromClassId", async () => {
        await expect(
            joinClassFromClassId("1", "1", "student")
        ).rejects.toThrowError("already in class");
        await expect(
            joinClassFromClassId("1", "1", "instructor")
        ).rejects.toThrowError("already in class");
        await expect(joinClassFromClassId("1", "4", "TA")).rejects.toThrowError(
            "already in class"
        );
        await expect(
            joinClassFromClassId("1", "20", "student")
        ).rejects.toThrowError("does not exist");
        await expect(
            joinClassFromClassId("21", "1", "student")
        ).rejects.toThrowError("does not exist");
        await expect(
